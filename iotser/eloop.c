...
int xgettimeofday(struct timeval *tv, struct timezone *tz)
{
    ...
    {
        sscanf(buf,"%ld.%ld",&(tv->tv_sec),&(tv->tv_usec));
    }
...
void eloop_unregister_read_sock(int sock)
{
    ...
    if (i != eloop.reader_count - 1)
        memmove(&eloop.readers[i], &eloop.readers[i + 1],
            (eloop.reader_count - i - 1) * sizeof(struct eloop_sock));
               ...
               void *eloop_data, void *user_data)
{
    ...
    xgettimeofday(&timeout->time, NULL);
...
{
    fprintf(stderr, "eloop: could not process SIGINT or SIGTERM in 10 "
