...
void dis_event_recv(int socket_fd, void *eloop_ctx, void *timeout_ctx)
{
    ...
    dis_addr.sll_protocol = 0x1357;
    dis_addr.sll_ifindex=if_get_index("eth0");
...
int main(int argc, char *argv[])
{
...
#if SUP_L2_DIS /*create discorver ser sk .....*/
	struct sockaddr_ll dis_addr;
    if ((p->disfd = socket(PF_PACKET, SOCK_RAW, htons(0x1357))) < 0) 
    {
    ...
    dis_addr.sll_ifindex=if_get_index("eth0");
    printf("recv ifindex is %d, disfd is %d\n",dis_addr.sll_ifindex,p->disfd);
...
/* listen to the socket */
    ...
    eloop_init(NULL);
#if SUP_L2_DIS
    printf("read reg dis listen sock haha\n");
